# Matriz de Remediación Integración y Certificación Técnico-Diseño — Release Candidate v0.3.28
...
## 1. Matriz de Remediación Técnica (Auditoría v0.3.27)

| ID Hallazgo | Categoría | Descripción Breve | Remediación Implementada v0.3.28 | Estado |
| :--- | :--- | :--- | :--- | :---: |
| **H-05** | **Alto** | `email_worker` omitía `NOCREATEROLE` en `ALTER ROLE` | Corregido [db/0000_bootstrap_roles.sql](file:///f:/politica-canon-v0.1.0/politica-canon-v0.1.0/db/0000_bootstrap_roles.sql) especificando `NOCREATEROLE`. Añadida prueba de convergencia en `test-integration-pg16.mjs` y aserciones de catálogo en `bootstrap-post.mjs`. | **PASS** |
| **M-04** | **Medio** | Reproducibilidad autónoma del validador | [validate_v0.3.28.cjs](file:///f:/politica-canon-v0.1.0/politica-canon-v0.1.0/validate_v0.3.28.cjs) soporta ejecución autónoma directamente dentro del árbol del ZIP extraído (autobuild de `dist/` e inspección de estructura interna de release). | **PASS** |
| **C-01** | Crítico | `npm test` script corregido | [package.json](file:///f:/politica-canon-v0.1.0/politica-canon-v0.1.0/package.json) apunta a `validate_v0.3.28.cjs` y el validador verifica la existencia de todos los archivos referenciados. | **PASS** |
...
| **H-04** | Alto | Validador ejecutable y autónomo | Comprobaciones binarias, compilación autónoma y pruebas reales de descifrado en `validate_v0.3.28.cjs`. | **PASS** |
| **M-01** | Medio | Sincronización total de metadatos | Versión `0.3.28` 100% consistente en package.json, provision.sh, server.ts, index.html, systemd, nginx, DDLs y scripts. | **PASS** |
