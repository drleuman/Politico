# Script PowerShell para la creación determinista e inmutable de politica-canon-v0.4.0-alpha.3.zip y MANIFEST_v0.4.0-alpha.3.json
$ErrorActionPreference = "Stop"

$version = "0.4.0-alpha.3"
$zipName = "politica-canon-v0.4.0-alpha.3.zip"
$rootDir = "politica-canon-v0.4.0-alpha.3"
$manifestName = "MANIFEST_v0.4.0-alpha.3.json"
...
# Re-calcular la lista completa de archivos en el zip (incluyendo RELEASE_FILES.json recién creado) para que el manifest refleje la cantidad real exacta de archivos (C-B01 fix)
$finalStagingFiles = Get-ChildItem -Path $stagingDir -Recurse -File | Sort-Object FullName
$realTotalFileCount = $finalStagingFiles.Count
...
foreach ($file in $finalStagingFiles) {
...
$manifestObj = [ordered]@{
    ...
    sha256 = $zipHashHex
    fileCount = $realTotalFileCount
    canonicalDatabaseEngine = "PostgreSQL 16+"
    dictamen = "PASS (RELEASE CANDIDATE V0.4.0-ALPHA.3 - FASE B COMPONENT ARCHITECTURE REMEDIATED)"
}
...
Write-Host "Artefacto $zipName creado exitosamente. Size: $($zipBytes.Length) bytes, SHA-256: $zipHashHex, Files: $realTotalFileCount"
