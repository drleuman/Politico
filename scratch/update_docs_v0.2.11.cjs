...
for (const doc of targetDocs) {
  if (fs.existsSync(doc)) {
    ...
    if (content.includes('0.2.10')) {
      content = content.replaceAll('v0.2.10', 'v0.2.11');
      content = content.replaceAll('0.2.10', '0.2.11');
      fs.writeFileSync(doc, content, 'utf8');
      updatedCount++;
      console.log(`Updated ${doc} to v0.2.11`);
    } else {
      console.log(`No v0.2.10 references found in ${doc}`);
    }
  } else {
