...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — FASE 0 (v0.2.11) ===\n');
...
// 2. Verificación Criptográfica Byte por Byte de los 19 Informes Históricos Intactos (SHA-256)
...
assertCheck(`Protección Trigger DB contra Mutación Directa de Status (C-03, H-01)`, hasProtectStatusTrigger);

const hasVerifyAuditChainCrypto = ddlContent.includes('FUNCTION verify_audit_chain') &&
  ddlContent.includes("digest(") &&
  ddlContent.includes("'sha256'") &&
  !ddlContent.includes("repeat('a', 64)");
assertCheck(`Función PL/pgSQL verify_audit_chain con Recalculación SHA-256 Genérica sin Excepción (C-01, C-04)`, hasVerifyAuditChainCrypto);
...
assertCheck(`Campo target_authority_body_id en Role Requests (C-05)`, hasTargetAuthorityBody);

const hasTransactionalCommands = ddlContent.includes('FUNCTION submit_document_draft_transactional') &&
  ddlContent.includes('FUNCTION freeze_document_submission_transactional') &&
  ddlContent.includes('FUNCTION approve_decision_transactional') &&
  ddlContent.includes('FUNCTION publish_document_transactional');
assertCheck(`Comandos Transaccionales Protegidos para Transiciones de Estado (H-01)`, hasTransactionalCommands);

const hasDecisionVotesTable = ddlContent.includes('CREATE TABLE decision_votes');
assertCheck(`Tabla decision_votes para Control Individual de Votos y Quórum (H-02)`, hasDecisionVotesTable);
...
assertCheck(`Asignación Real de Propiedad de Esquema y Objetos a app_owner (C-03)`, hasOwnerAssignment);
...
// 8. Análisis Dinámico del Diagrama Mermaid en docs/architecture/ERD.md (30 Entidades)
...
// 9. Verificación de Versiones Documentales Unificadas a v0.2.11
...
for (const d of targetDocs) {
  if (fs.existsSync(d)) {
    ...
    if (text.includes('0.2.11')) {
      updatedDocsCount++;
    }
  }
}
assertCheck(`Actualización de Línea Base Documental a v0.2.11 (${updatedDocsCount}/${targetDocs.length})`, updatedDocsCount === targetDocs.length);

console.log('\n---------------------------------------------------');
console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
if (failCount === 0) {
  console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA v0.2.11: PASS');
  process.exit(0);
} else {
  console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA v0.2.11: FAIL');
  process.exit(1);
}
