...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — FASE 0 (v0.2.9) ===\n');
...
// 2. Verificación Criptográfica Byte por Byte de los 15 Informes Históricos Intactos (SHA-256)
...
// 3. Verificación de DDL e Integridad de Esquema SQL (0001_initial_schema.sql)
...
const hasVerifyAuditChain = ddlContent.includes('FUNCTION verify_audit_chain') && ddlContent.includes('SECURITY DEFINER');
assertCheck(`Función PL/pgSQL verify_audit_chain Incorporada (H-03)`, hasVerifyAuditChain);

const hasPendingOutboxTenants = ddlContent.includes('FUNCTION get_pending_outbox_tenants') && ddlContent.includes('SECURITY DEFINER');
assertCheck(`Función PL/pgSQL get_pending_outbox_tenants para RLS Worker (C-03)`, hasPendingOutboxTenants);

const hasDraftCommentsFK = ddlContent.includes('FOREIGN KEY (organization_id, draft_id) REFERENCES working_drafts(organization_id, id)');
assertCheck(`Integridad Multitenant Compuesta en draft_comments (H-04)`, hasDraftCommentsFK);

const hasPublicationsConstraint = ddlContent.includes('unique_org_publication_decision UNIQUE (organization_id, id, decision_id)');
assertCheck(`Restricción DDL Corregida en publications (C-02)`, hasPublicationsConstraint);

const hasRoleAssignmentsFK = ddlContent.includes('FOREIGN KEY (organization_id, request_id) REFERENCES role_assignment_requests(organization_id, id)');
assertCheck(`FK Compuesta Corregida en role_assignments (C-02)`, hasRoleAssignmentsFK);

// 4. Verificación de Migración de Reversión (Down Migration)
const hasDownMigration = fs.existsSync('db/migrations/0001_initial_schema_down.sql');
let downDropCount = 0;
if (hasDownMigration) {
  const downContent = fs.readFileSync('db/migrations/0001_initial_schema_down.sql', 'utf8');
  downDropCount = (downContent.match(/DROP TABLE IF EXISTS/gi) || []).length;
}
assertCheck(`Entrega y Cobertura de Script Rollback (0001_initial_schema_down.sql)`, hasDownMigration && downDropCount === 29, `Tablas eliminadas en rollback: ${downDropCount}/29`);

// 5. Verificación de Orden de Inicialización en Docker Compose y Privilegios
const dockerContent = fs.readFileSync('docker-compose.audit.yml', 'utf8');
const isCorrectInitOrder = dockerContent.includes('0001_initial_schema.sql:/docker-entrypoint-initdb.d/01_initial_schema.sql') &&
  dockerContent.includes('bootstrap_roles.sql:/docker-entrypoint-initdb.d/02_bootstrap_roles.sql');
assertCheck(`Orden de Inicialización Docker Compose (Schema 01 -> Bootstrap 02) (C-01)`, isCorrectInitOrder);

const bootstrapContent = fs.readFileSync('db/bootstrap_roles.sql', 'utf8');
const hasRevokedDirectDml = bootstrapContent.includes('REVOKE INSERT, UPDATE, DELETE ON role_assignments, decisions, publications');
assertCheck(`Revocación de Permisos de Escritura Directa en Tablas Sensibles (C-04)`, hasRevokedDirectDml);

// 6. Ejecución Semántica Real del Evaluador TypeScript y Contrato de Autorización (Node 24)
...
try {
  ...
  if (result.total === 5 && result.passed === 5 && result.failed === 0) {
    tsAuthPassed = true;
  }
} catch (err) {
  console.log(`  -> Error ejecutando pruebas de autorización TS: ${err.message}`);
}
assertCheck(`Ejecución Semántica Real de Pruebas de Autorización TypeScript (C-05)`, tsAuthPassed);

// 7. Análisis Dinámico del Diagrama Mermaid en docs/architecture/ERD.md (29 Entidades)
...
// 8. Verificación de Versiones Documentales Unificadas a v0.2.9
...
for (const d of targetDocs) {
  if (fs.existsSync(d)) {
    ...
    if (text.includes('0.2.9')) {
      updatedDocsCount++;
    }
  }
}
assertCheck(`Actualización de Línea Base Documental a v0.2.9 (${updatedDocsCount}/${targetDocs.length})`, updatedDocsCount === targetDocs.length);

console.log('\n---------------------------------------------------');
console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
if (failCount === 0) {
  console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA v0.2.9: PASS');
  process.exit(0);
} else {
  console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA v0.2.9: FAIL');
  process.exit(1);
}
