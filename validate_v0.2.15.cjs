...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — FASE 0 (v0.2.15) ===\n');
...
// 5. Coincidencia Exacta de Firmas y Revocación de INSERT Directo (C-01, C-03)
...
assertCheck(`Coincidencia de Firmas DDL / Bootstrap y Funciones de Voto (C-01, C-03)`, hasExactSignaturesInGrant);
...
assertCheck(`Revocación de INSERT Directo en decision_votes a app_user (C-03)`, hasRevokedDirectVoteInsert);

// 6. Vector Criptográfico Canónico SHA-256 JCS Anidado con Notación Exponencial y Unicode (C-02)
...
assertCheck(`Vector Criptográfico Canónico SHA-256 JCS con Unicode Supletorio y Exponenciales (C-02)`, sharedVectorPassed);

// 7. Validaciones Server-Side y Columnas Físicas DDL (C-01, C-02, C-03)
const hasPhysicalColumns = ddlContent.includes('is_active BOOLEAN NOT NULL DEFAULT TRUE') &&
  ddlContent.includes('valid_from TIMESTAMPTZ NOT NULL DEFAULT NOW()') &&
  ddlContent.includes('valid_until TIMESTAMPTZ');
assertCheck(`Columnas Físicas is_active y valid_from / valid_until en Esquema (C-01)`, hasPhysicalColumns);
...
const hasServerSideChecks = ddlContent.includes('CROSS_TENANT_VIOLATION') &&
  ...
  ddlContent.includes('QUORUM_NOT_REACHED');
assertCheck(`Validaciones Server-Side de Organización, Membresía y Quórum Colegiado (C-01, C-03)`, hasServerSideChecks);

// 8. Claves Foráneas Compuestas y JCS UTF-16BE (C-01, C-02, H-01)
...
const hasJcsUtf16Canonicalizer = ddlContent.includes('FUNCTION jcs_canonicalize_jsonb(p_val JSONB)') &&
  ddlContent.includes('ORDER BY convert_to(key, \'UTF16BE\') ASC');
assertCheck(`Función PL/pgSQL jcs_canonicalize_jsonb RFC 8785 con UTF16BE en DDL (C-02)`, hasJcsUtf16Canonicalizer);
...
// 12. Verificación de Versiones Documentales Unificadas a v0.2.15 (H-03)
...
for (const d of targetDocs) {
  if (fs.existsSync(d)) {
    ...
    if (text.includes('0.2.15')) {
      updatedDocsCount++;
    }
  }
}
assertCheck(`Actualización de Línea Base Documental a v0.2.15 (${updatedDocsCount}/${targetDocs.length}) (H-03)`, updatedDocsCount === targetDocs.length);

console.log('\n---------------------------------------------------');
console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
if (failCount === 0) {
  console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA v0.2.15: PASS');
  process.exit(0);
} else {
  console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA v0.2.15: FAIL');
  process.exit(1);
}
