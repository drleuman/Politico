...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.1 (FASE 1 MVP) ===\n');
...
async function runValidation() {
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (37 Informes)
  ...
  // 4. Verificación de Artefactos de Despliegue y Scripts Versionados (C-02)
  ...
  for (const f of requiredDeployFiles) {
    if (!fs.existsSync(f)) {
      missingDeploy++;
      console.log(`  -> Artefacto de despliegue/script faltante: ${f}`);
    }
  }
  assertCheck('Artefactos de Despliegue y Scripts (bootstrap/migrate) Versionados', missingDeploy === 0, `${missingDeploy} archivos faltantes`);

  // 5. Prueba de Integración del Migrador (Advisory Lock & Fail-Closed en Checksum Mismatch)
  let migradorIntegrityPass = false;
  try {
    const migrateContent = fs.readFileSync('scripts/migrate-production.mjs', 'utf8');
    const hasAdvisoryLock = migrateContent.includes('pg_advisory_lock') && migrateContent.includes('pg_advisory_unlock');
    const hasChecksumCheck = migrateContent.includes('CHECKSUM_MISMATCH') && migrateContent.includes('process.exit(1)');
    migradorIntegrityPass = hasAdvisoryLock && hasChecksumCheck;
  } catch (err) {
    console.log('  -> Error verificando script migrador:', err.message);
  }
  assertCheck('Migrador con Advisory Lock e Inspección Fail-Closed de Checksums (C-02)', migradorIntegrityPass);

  // 6. Pruebas HTTP Reales Fastify & Probes
  ...
  try {
    ...
    const homeOk = homeRes.statusCode === 200 && homeRes.payload.includes('Acceso Restringido');
  ...
  assertCheck('Pruebas HTTP Reales (/healthz, /readyz, Portada Intranet Protegida)', probeTestPass);

  // 7. Pruebas PGlite en PostgreSQL 16 (DDL + Bootstrap + JCS)
  const db = new PGlite();
  try {
    const ddlRaw = fs.readFileSync('db/migrations/0001_initial_schema.sql', 'utf8');
    const ddlExecutable = ddlRaw.replace(/CREATE EXTENSION IF NOT EXISTS\s+("?pgcrypto"?);?/gi, '-- pgcrypto native in PG16');
    await db.exec(ddlExecutable);
    await db.exec(fs.readFileSync('db/bootstrap_roles.sql', 'utf8'));

    // Vector JCS C-01
    const jcsRes = await db.query(`SELECT jcs_canonicalize_jsonb('{"small":1e-7,"big":1e+21,"normal":100}'::jsonb) AS formatted;`);
    const jcsExpected = '{"big":1e+21,"normal":100,"small":1e-7}';
    const jcsPass = jcsRes.rows[0].formatted === jcsExpected;

    assertCheck('PostgreSQL 16 PGlite DDL & Formateador JCS RFC 8785 (C-01)', jcsPass);
  } catch (err) {
    assertCheck('PostgreSQL 16 PGlite DDL & Formateador JCS RFC 8785 (C-01)', false, err.message);
  } finally {
    await db.close();
  }
  ...
  if (failCount === 0) {
    console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.1: PASS');
  } else {
    console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.1: FAIL');
    process.exit(1);
  }
}
