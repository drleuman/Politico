...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.10 (FASE 1 MVP) ===\n');
...
async function runValidation() {
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (55 Informes y Matrices Completo)
  ...
  // 4. Coherencia de Metadatos de Release v0.3.10
  ...
  const dbClientTs = fs.readFileSync('src/db/client.ts', 'utf8');
  const bootstrapPermissionsSql = fs.readFileSync('db/0002_bootstrap_permissions.sql', 'utf8');
  const bootstrapRolesSql = fs.readFileSync('db/0000_bootstrap_roles.sql', 'utf8');

  const metaVersion = packageJson.version === '0.3.10' &&
    nginxConf.includes('v0.3.10') &&
    systemdService.includes('v0.3.10') &&
    provisionSh.includes('v0.3.10') &&
    indexHtml.includes('v0.3.10') &&
    deploymentReport.includes('v0.3.10');

  assertCheck('Coherencia de Metadatos de Release v0.3.10', metaVersion, `package.json=${packageJson.version}`);

  // 5. Verificación de Correcciones Técnicas Específicas v0.3.10
  // Sonda de salud usa pg_roles y rolsuper
  const pgRolesHealthCheck = dbClientTs.includes('FROM pg_roles') && dbClientTs.includes('rolsuper AS is_superuser') && dbClientTs.includes('rolname = current_user');
  assertCheck('Consulta de Sonda de Salud sobre pg_roles y rolsuper (Requisito 1)', pgRolesHealthCheck);

  // Post-bootstrap incluye GRANT CONNECT ON DATABASE
  const grantConnectCheck = bootstrapPermissionsSql.includes('GRANT CONNECT ON DATABASE politica_canon TO politica_canon_app;');
  assertCheck('GRANT CONNECT ON DATABASE en Post-Bootstrap (Requisito 2)', grantConnectCheck);

  // Pre-bootstrap incluye CREATE EXTENSION IF NOT EXISTS pgcrypto
  const pgcryptoCheck = bootstrapRolesSql.includes('CREATE EXTENSION IF NOT EXISTS pgcrypto;');
  assertCheck('CREATE EXTENSION IF NOT EXISTS pgcrypto en Pre-Bootstrap (Requisito 3)', pgcryptoCheck);

  // Nginx conf en Ámbito de Servidor sin location / duplicado (Requisito 7)
  const nginxNoDuplicateLocation = !/^\s*location\s+\/\s*\{/m.test(nginxConf) && nginxConf.includes('auth_basic "Acceso Restringido — Intranet Política Canon";');
  assertCheck('Directivas Nginx en Ámbito de Servidor sin location / duplicado (Requisito 7)', nginxNoDuplicateLocation);

  // 6. Integración PostgreSQL PGlite Basal
  try {
    const db = new PGlite();
    ...
    // Fase 2: Migración DDL ejecutando SET ROLE app_owner
    ...
    assertCheck('Inicialización de 3 Fases y Permisos DML (PGlite Basal)', dmlDenied);
  } catch (err) {
    assertCheck('Inicialización de 3 Fases y Permisos DML (PGlite Basal)', false, err.message);
  }

  // 7. Verificación de Inclusión Directa de Integración Real en npm test
  const testScript = packageJson.scripts['test'] || '';
  const testScriptIncludesIntegration = testScript.includes('node scripts/test-integration-pg16.mjs') && testScript.includes('node validate_v0.3.10.cjs');

  assertCheck('Inclusión Directa de Integración Real en npm test (Requisito v0.3.10)', testScriptIncludesIntegration);

  console.log(`\n---------------------------------------------------`);
  console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
  console.log(`DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.10: ${failCount === 0 ? 'PASS' : 'FAIL'}\n`);
