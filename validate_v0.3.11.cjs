...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.11 (FASE 1 MVP) ===\n');
...
async function runValidation() {
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (57 Informes y Matrices Completo)
  ...
  // 4. Coherencia de Metadatos de Release v0.3.11
  ...
  const metaVersion = packageJson.version === '0.3.11' &&
    nginxConf.includes('v0.3.11') &&
    systemdService.includes('v0.3.11') &&
    provisionSh.includes('v0.3.11') &&
    indexHtml.includes('v0.3.11') &&
    deploymentReport.includes('v0.3.11');

  assertCheck('Coherencia de Metadatos de Release v0.3.11', metaVersion, `package.json=${packageJson.version}`);
  ...
  const groupPermsCheck = provisionSh.includes('usermod -aG politica-canon postgres') &&
    ...
    deploymentReport.includes('usermod -aG politica-canon postgres');
  assertCheck('Modelo de Permisos Restringido 0750 / 0640 y Pertenencia a Grupo (Bloqueante B-02)', groupPermsCheck);

  // 6. Verificación de Correcciones Técnicas Previa (Sonda pg_roles, GRANT CONNECT, pgcrypto)
  ...
  assertCheck('Consulta de Sonda de Salud sobre pg_roles y rolsuper', pgRolesHealthCheck);

  const grantConnectCheck = bootstrapPermissionsSql.includes('GRANT CONNECT ON DATABASE politica_canon TO politica_canon_app;');
  assertCheck('GRANT CONNECT ON DATABASE en Post-Bootstrap', grantConnectCheck);

  const pgcryptoCheck = bootstrapRolesSql.includes('CREATE EXTENSION IF NOT EXISTS pgcrypto;');
  assertCheck('CREATE EXTENSION IF NOT EXISTS pgcrypto en Pre-Bootstrap', pgcryptoCheck);

  // 7. Integración PostgreSQL PGlite Basal
  try {
    const db = new PGlite();
  ...
  // 8. Verificación de Inclusión Directa de Integración Real en npm test (B-01)
  const testScript = packageJson.scripts['test'] || '';
  const testScriptIncludesIntegration = testScript.includes('node scripts/test-integration-pg16.mjs') && testScript.includes('node validate_v0.3.11.cjs');

  assertCheck('Inclusión Directa de Integración Real en npm test (Bloqueante B-01)', testScriptIncludesIntegration);

  console.log(`\n---------------------------------------------------`);
  console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
  console.log(`DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.11: ${failCount === 0 ? 'PASS' : 'FAIL'}\n`);
