...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.12 (FASE 1.1) ===\n');
...
async function runValidation() {
  ...
  for (const file of markdownFiles) {
    ...
    while ((match = linkRegex.exec(content)) !== null) {
      ...
      if (linkPath.startsWith('http://') || linkPath.startsWith('https://') || linkPath.startsWith('file://') || linkPath.startsWith('#') || linkPath.startsWith('mailto:')) {
        continue;
      }
      totalLinks++;
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (58 Informes y Matrices Completo)
  ...
  // 4. Coherencia de Metadatos de Release v0.3.12
  ...
  const metaVersion = packageJson.version === '0.3.12' &&
    nginxConf.includes('v0.3.12') &&
    systemdService.includes('v0.3.12') &&
    provisionSh.includes('v0.3.12') &&
    indexHtml.includes('v0.3.12');

  assertCheck('Coherencia de Metadatos de Release v0.3.12', metaVersion, `package.json=${packageJson.version}`);
  ...
  // 6. Integración PostgreSQL PGlite Basal con Migración DDL 0003
  try {
    const db = new PGlite();
  ...
  // 7. Verificación de Inclusión Directa de Integración Real en npm test (B-01)
  const testScript = packageJson.scripts['test'] || '';
  const testScriptIncludesIntegration = testScript.includes('node scripts/test-integration-pg16.mjs') && testScript.includes('node validate_v0.3.12.cjs');
  ...
  console.log(`DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.12: ${failCount === 0 ? 'PASS' : 'FAIL'}\n`);
