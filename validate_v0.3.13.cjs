...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.13 (FASE 1.1 CORRECTIVA) ===\n');
...
async function runValidation() {
  // 1. Verificación de Enlaces Relativos Markdown y Ausencia de Rutas Locales file:///
  let brokenLinks = 0;
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (61 Informes y Matrices Completo)
  ...
  // 4. Coherencia de Metadatos de Release v0.3.13
  ...
  const metaVersion = packageJson.version === '0.3.13' &&
    nginxConf.includes('v0.3.13') &&
    systemdService.includes('v0.3.13') &&
    provisionSh.includes('v0.3.13') &&
    indexHtml.includes('v0.3.13');

  assertCheck('Coherencia de Metadatos de Release v0.3.13', metaVersion, `package.json=${packageJson.version}`);
  ...
  // 6. Integración PostgreSQL PGlite Basal con Migración DDL 0003 y Funciones SECURITY DEFINER
  try {
    const db = new PGlite();
    ...
    assertCheck('Inicialización Basal PGlite con Migración Incremental 0003 y Permisos DML', dmlDenied);
  } catch (err) {
    assertCheck('Inicialización Basal PGlite con Migración Incremental 0003 y Permisos DML', false, err.message);
  }

  // 7. Verificación de Inclusión Directa de Integración Real en npm test (B-01 & v0.3.13)
  const testScript = packageJson.scripts['test'] || '';
  const testScriptIncludesIntegration = testScript.includes('node scripts/test-integration-pg16.mjs') && testScript.includes('node validate_v0.3.13.cjs');

  assertCheck('Inclusión Directa de Integración Real en npm test (Bloqueante B-01 & v0.3.13)', testScriptIncludesIntegration);

  console.log(`\n---------------------------------------------------`);
  console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
  console.log(`DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.13: ${failCount === 0 ? 'PASS' : 'FAIL'}\n`);
