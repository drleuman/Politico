...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.15 (FASE 1.1 CORRECTIVA) ===\n');
...
async function runValidation() {
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (65 Informes y Matrices Completo)
  ...
  // 4. Coherencia de Metadatos de Release v0.3.15
  ...
  const metaVersion = packageJson.version === '0.3.15' &&
    nginxConf.includes('v0.3.15') &&
    systemdService.includes('v0.3.15') &&
    provisionSh.includes('v0.3.15') &&
    indexHtml.includes('v0.3.15');

  assertCheck('Coherencia de Metadatos de Release v0.3.15', metaVersion, `package.json=${packageJson.version}`);
  ...
  // 7. Verificación de Inclusión Directa de Integración Real en npm test (B-01 & v0.3.15)
  const testScript = packageJson.scripts['test'] || '';
  const testScriptIncludesIntegration = testScript.includes('node scripts/test-integration-pg16.mjs') && testScript.includes('node validate_v0.3.15.cjs');

  assertCheck('Inclusión Directa de Integración Real en npm test (Bloqueante B-01 & v0.3.15)', testScriptIncludesIntegration);

  console.log(`\n---------------------------------------------------`);
  console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
  console.log(`DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.15: ${failCount === 0 ? 'PASS' : 'FAIL'}\n`);
