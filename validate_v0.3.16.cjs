...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.16 (FASE 1.1 CORRECTIVA) ===\n');
...
async function runValidation() {
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (67 Informes y Matrices Completo)
  ...
  // 4. Coherencia de Metadatos y Verificación de Scripts de Despliegue en package.json (v0.3.16)
  ...
  const metaVersion = packageJson.version === '0.3.16' &&
    nginxConf.includes('v0.3.16') &&
    systemdService.includes('v0.3.16') &&
    provisionSh.includes('v0.3.16') &&
    indexHtml.includes('v0.3.16');
  ...
  assertCheck('Coherencia de Metadatos de Release v0.3.16 y Scripts Canónicos de Despliegue', metaVersion && deploymentScriptsOk, `version=${packageJson.version}, scriptsOk=${deploymentScriptsOk}`);
  ...
  // 7. Verificación de Inclusión Directa de Integración Real en npm test (B-01 & v0.3.16)
  const testScript = packageJson.scripts['test'] || '';
  const testScriptIncludesIntegration = testScript.includes('node scripts/test-integration-pg16.mjs') && testScript.includes('node validate_v0.3.16.cjs');

  assertCheck('Inclusión Directa de Integración Real en npm test (Bloqueante B-01 & v0.3.16)', testScriptIncludesIntegration);

  console.log(`\n---------------------------------------------------`);
  console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
  console.log(`DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.16: ${failCount === 0 ? 'PASS' : 'FAIL'}\n`);
