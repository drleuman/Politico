...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.18 (FASE 1.1 FUNCIONAL) ===\n');
...
async function runValidation() {
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (69 Informes y Matrices Completo)
  const historicalHashes = {
    ...
    'REMEDIATION_MATRIX_v0.3.17.md': crypto.createHash('sha256').update(fs.readFileSync('REMEDIATION_MATRIX_v0.3.17.md', 'utf8').replace(/\r\n/g, '\n'), 'utf8').digest('hex'),
    ...
    'VALIDATION_REPORT_v0.3.17.md': crypto.createHash('sha256').update(fs.readFileSync('VALIDATION_REPORT_v0.3.17.md', 'utf8').replace(/\r\n/g, '\n'), 'utf8').digest('hex')
  };
  ...
  // 3. Compilación TypeScript Estricta sin Errores
  try {
  ...
  // 4. Coherencia Estricta de Metadatos de Release v0.3.18
  const packageJson = JSON.parse(fs.readFileSync('package.json', 'utf8'));
  const version = packageJson.version;
  const isVersionCorrect = version === '0.3.18';

  const serverTs = fs.readFileSync('src/server.ts', 'utf8');
  const isServerVersionCorrect = serverTs.includes('v0.3.18');

  const migratorMjs = fs.readFileSync('scripts/migrate-production.mjs', 'utf8');
  const isMigratorVersionCorrect = migratorMjs.includes('v0.3.18');

  const integrationMjs = fs.readFileSync('scripts/test-integration-pg16.mjs', 'utf8');
  const isIntegrationVersionCorrect = integrationMjs.includes('v0.3.18');

  const systemdService = fs.readFileSync('deploy/systemd/politica-canon.service', 'utf8');
  const isSystemdVersionCorrect = systemdService.includes('v0.3.18');

  const vhostNginx = fs.readFileSync('deploy/plesk/vhost_nginx.conf', 'utf8');
  const isNginxVersionCorrect = vhostNginx.includes('v0.3.18');

  const indexHtml = fs.readFileSync('public/index.html', 'utf8');
  const isIndexHtmlVersionCorrect = indexHtml.includes('v0.3.18');

  const changelog = fs.readFileSync('CHANGELOG.md', 'utf8');
  const isChangelogVersionCorrect = changelog.includes('[0.3.18]');

  const isMetadataConsistent = 
    isVersionCorrect &&
    isServerVersionCorrect &&
    isMigratorVersionCorrect &&
    isIntegrationVersionCorrect &&
    isSystemdVersionCorrect &&
    isNginxVersionCorrect &&
    isIndexHtmlVersionCorrect &&
    isChangelogVersionCorrect;

  assertCheck(
    'Coherencia de Metadatos de Release v0.3.18 y Scripts Canónicos de Despliegue',
    isMetadataConsistent,
    `package.json (${version}), server.ts (${isServerVersionCorrect}), migrate (${isMigratorVersionCorrect}), test (${isIntegrationVersionCorrect}), systemd (${isSystemdVersionCorrect}), nginx (${isNginxVersionCorrect}), index.html (${isIndexHtmlVersionCorrect}), changelog (${isChangelogVersionCorrect})`
  );

  // 5. Modelo de Permisos Restringido 0750 / 0640 y Pertenencia a Grupo (B-02)
  const provisionSh = fs.readFileSync('deploy/scripts/provision.sh', 'utf8');
  const groupPermsCheck = 
    provisionSh.includes('usermod -aG politica-canon postgres') &&
  ...
  // 6. Inicialización Basal PGlite con Migraciones DDL 0003/0004/0005 y Resolver por Hash token_resolver
  try {
    const db = new PGlite();
  ...
  // 7. Verificación de Inclusión Directa de Integración Real en npm test (B-01 & v0.3.18)
  const testScript = packageJson.scripts['test'] || '';
  const testScriptIncludesIntegration = testScript.includes('node scripts/test-integration-pg16.mjs') && testScript.includes('node validate_v0.3.18.cjs');

  assertCheck('Inclusión Directa de Integración Real en npm test (Bloqueante B-01 & v0.3.18)', testScriptIncludesIntegration);

  console.log(`\n---------------------------------------------------`);
  console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
  console.log(`DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.18: ${failCount === 0 ? 'PASS' : 'FAIL'}\n`);
