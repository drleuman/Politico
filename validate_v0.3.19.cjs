...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.19 (FASE 1.1 FUNCIONAL REMEDIADA) ===\n');
...
async function runStaticValidation() {
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (Con normalización CRLF/LF)
  ...
  if (fs.existsSync('REMEDIATION_MATRIX_v0.3.17.md')) {
    historicalHashes['REMEDIATION_MATRIX_v0.3.17.md'] = crypto.createHash('sha256').update(fs.readFileSync('REMEDIATION_MATRIX_v0.3.17.md', 'utf8').replace(/\r\n/g, '\n'), 'utf8').digest('hex');
  }
  if (fs.existsSync('VALIDATION_REPORT_v0.3.17.md')) {
    historicalHashes['VALIDATION_REPORT_v0.3.17.md'] = crypto.createHash('sha256').update(fs.readFileSync('VALIDATION_REPORT_v0.3.17.md', 'utf8').replace(/\r\n/g, '\n'), 'utf8').digest('hex');
  }
  if (fs.existsSync('REMEDIATION_MATRIX_v0.3.18.md')) {
    historicalHashes['REMEDIATION_MATRIX_v0.3.18.md'] = crypto.createHash('sha256').update(fs.readFileSync('REMEDIATION_MATRIX_v0.3.18.md', 'utf8').replace(/\r\n/g, '\n'), 'utf8').digest('hex');
  }
  if (fs.existsSync('VALIDATION_REPORT_v0.3.18.md')) {
    historicalHashes['VALIDATION_REPORT_v0.3.18.md'] = crypto.createHash('sha256').update(fs.readFileSync('VALIDATION_REPORT_v0.3.18.md', 'utf8').replace(/\r\n/g, '\n'), 'utf8').digest('hex');
  }
  ...
  // 4. Coherencia de Metadatos de Release v0.3.19
  const pkg = JSON.parse(fs.readFileSync('package.json', 'utf8'));
  const versionOk = pkg.version === '0.3.19';
  const provisionContent = fs.readFileSync('deploy/scripts/provision.sh', 'utf8');
  const provisionVersionOk = provisionContent.includes('v0.3.19');
  const migratorContent = fs.readFileSync('scripts/migrate-production.mjs', 'utf8');
  const migratorVersionOk = migratorContent.includes('v0.3.19');

  assertCheck(
    'Coherencia de Metadatos de Release v0.3.19 y Scripts Canónicos de Despliegue',
  ...
  assertCheck('Inclusión Directa de Integración Real en npm test (Bloqueante B-01 & v0.3.19)', testScriptOk);

  console.log('\n---------------------------------------------------');
  console.log(`TOTAL CHECKS: ${totalChecks} | PASS: ${passedChecks} | FAIL: ${totalChecks - passedChecks}`);
  if (passedChecks === totalChecks) {
    console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.19: PASS\n');
  } else {
    console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.19: FAIL\n');
    process.exit(1);
  }
}
