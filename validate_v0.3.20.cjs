...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.20 (FASE 1.1 FUNCIONAL REMEDIADA CERTIFICADA) ===\n');
...
async function runStaticValidation() {
  // 1. Verificación de Enlaces Relativos Portables Markdown
  let totalLinks = 0;
  let brokenLinks = 0;
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos y Verificación Rigurosa de Manifiesto (H-01)
  const historicalHashes = {
    ...
    'REMEDIATION_MATRIX_v0.3.19.md': 'c18ed2118847bed597fd52bca619e3bc1a9bd5abf551435311e78b15f1779d17',
    ...
    'VALIDATION_REPORT_v0.3.19.md': '620236491e2d62c2f2bb5fc2b780fb9589578db5179cef98e3e415cc68a2c933',
    'REMEDIATION_MATRIX_v0.3.20.md': 'dd136abf9b169ec3189d17fab04a736178f0e85daeee77030513f202e339d654',
    'VALIDATION_REPORT_v0.3.20.md': '8dd8fc0c0c44c789ff7e98b09b82381f769cf4b69837dfbb3e19a1e43c991a48',
  };
  ...
  for (const [filePath, expectedHash] of Object.entries(historicalHashes)) {
    ...
    const actualHash = crypto.createHash('sha256').update(normalizedContent, 'utf8').digest('hex');
    if (actualHash !== expectedHash) {
      hashFailures++;
      console.log(`  -> Alteración criptográfica en ${filePath}: esperado ${expectedHash}, observado ${actualHash}`);
    } else {
      verifiedCount++;
    }
  }
  ...
  if (!isRepoMode) {
    const manifestPath = fs.existsSync('MANIFEST_v0.3.20.json') ? 'MANIFEST_v0.3.20.json' : (fs.existsSync('../MANIFEST_v0.3.20.json') ? '../MANIFEST_v0.3.20.json' : null);
    if (!manifestPath) {
      historicalPass = false;
      console.log('  -> Fallo H-01: Paquete ZIP no contiene informes históricos ni manifiesto externalizado MANIFEST_v0.3.20.json');
    } else {
      try {
        const manifestObj = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
        if (manifestObj.version !== '0.3.20' || manifestObj.packageName !== 'politica-canon-v0.3.20.zip' || !manifestObj.sha256) {
          historicalPass = false;
          console.log(`  -> Fallo H-01: MANIFEST_v0.3.20.json inválido o inconsistente: ${JSON.stringify(manifestObj)}`);
        }
      } catch (mErr) {
        historicalPass = false;
        console.log(`  -> Fallo H-01: Error al parsear MANIFEST_v0.3.20.json: ${mErr.message}`);
      }
    }
  }
  ...
  try {
    ...
    console.log('  -> Error de compilación TypeScript:', tscErr.stdout?.toString());
  }
  assertCheck('Compilación TypeScript Estricta (tsc --noEmit) sin Errores', tscOk);

  // 4. Coherencia de Metadatos de Release v0.3.20
  const pkg = JSON.parse(fs.readFileSync('package.json', 'utf8'));
  const versionOk = pkg.version === '0.3.20';
  const provisionContent = fs.readFileSync('deploy/scripts/provision.sh', 'utf8');
  const provisionVersionOk = provisionContent.includes('v0.3.20');
  const migratorContent = fs.readFileSync('scripts/migrate-production.mjs', 'utf8');
  const migratorVersionOk = migratorContent.includes('v0.3.20');

  assertCheck(
    'Coherencia de Metadatos de Release v0.3.20 y Scripts Canónicos de Despliegue',
    versionOk && provisionVersionOk && migratorVersionOk,
    `package.json: ${pkg.version}, provision.sh: ${provisionVersionOk}, migrate: ${migratorVersionOk}`
  );

  // 5. Modelo de Permisos Restringido 0750 / 0640 (B-02)
  const provisionHasPerms = provisionContent.includes('chmod 0750') && provisionContent.includes('chmod 0640');
  assertCheck('Modelo de Permisos Restringido 0750 / 0640 y Pertenencia a Grupo (B-02)', provisionHasPerms);

  // 6. Inicialización Basal PGlite con Migración Incremental 0005 y Permisos DML
  try {
    ...
    const migration0005Raw = fs.readFileSync('db/migrations/0005_fase_1_1_functional.sql', 'utf8');
    ...
    await db.exec(migration0005Raw);
    ...
    assertCheck('Inicialización Basal PGlite con Migración Incremental 0005 y Permisos DML', dmlDenied);
  } catch (err) {
    assertCheck('Inicialización Basal PGlite con Migración Incremental 0005 y Permisos DML', false, err.message);
  }

  // 7. Inclusión Directa de Integración Real en npm test
  const testScriptOk = pkg.scripts && pkg.scripts.test && pkg.scripts.test.includes('scripts/test-integration-pg16.mjs');
  assertCheck('Inclusión Directa de Integración Real en npm test (Bloqueante B-01 & v0.3.20)', testScriptOk);

  console.log('\n---------------------------------------------------');
  console.log(`TOTAL CHECKS: ${totalChecks} | PASS: ${passedChecks} | FAIL: ${totalChecks - passedChecks}`);
  if (passedChecks === totalChecks) {
    console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.20: PASS\n');
  } else {
    console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.20: FAIL\n');
    process.exit(1);
  }
}

runStaticValidation().catch((err) => {
  console.error('Error en validación estática:', err);
  process.exit(1);
});
