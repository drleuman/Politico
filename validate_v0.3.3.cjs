...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.3 (FASE 1 MVP) ===\n');
...
async function runValidation() {
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (41 Informes)
  ...
  // 4. Verificación de Artefactos y Scripts en 3 Fases (C-01, C-02, C-03, C-04, C-05)
  const requiredDeployFiles = [
    'deploy/systemd/politica-canon.service',
    'deploy/plesk/vhost_nginx.conf',
    'deploy/scripts/provision.sh',
    'db/0000_bootstrap_roles.sql',
    'db/0002_bootstrap_permissions.sql',
    'scripts/bootstrap-pre.mjs',
    'scripts/migrate-production.mjs',
    'scripts/bootstrap-post.mjs'
  ];

  let missingDeploy = 0;
  for (const f of requiredDeployFiles) {
    if (!fs.existsSync(f)) {
      missingDeploy++;
      console.log(`  -> Artefacto/script de despliegue faltante: ${f}`);
    }
  }
  assertCheck('Artefactos de Despliegue y Scripts en 3 Fases Versionados', missingDeploy === 0, `${missingDeploy} archivos faltantes`);

  // 5. Prueba de Integración de las 7 Condiciones de Aceptación en PGlite WASM
  let sevenConditionsPass = false;
  const db = new PGlite();
  try {
    // Fase 1: Pre-Bootstrap
    await db.exec(fs.readFileSync('db/0000_bootstrap_roles.sql', 'utf8'));

    // Fase 2: Migración DDL
    const ddlRaw = fs.readFileSync('db/migrations/0001_initial_schema.sql', 'utf8');
    const ddlExecutable = ddlRaw.replace(/CREATE EXTENSION IF NOT EXISTS\s+("?pgcrypto"?);?/gi, '-- pgcrypto native');
    await db.exec(ddlExecutable);

    // Fase 3: Post-Bootstrap
    await db.exec(fs.readFileSync('db/0002_bootstrap_permissions.sql', 'utf8'));

    // Condición 1: Comprobar que schema_migrations y tables existen
    const tableCheck = await db.query(`SELECT COUNT(*) AS count FROM information_schema.tables WHERE table_schema = 'public';`);
    const tablesCount = parseInt(tableCheck.rows[0].count, 10);

    // Condición 2: Comprobar JCS RFC 8785
    const jcsRes = await db.query(`SELECT jcs_canonicalize_jsonb('{"small":1e-7,"big":1e+21,"normal":100}'::jsonb) AS formatted;`);
    const jcsPass = jcsRes.rows[0].formatted === '{"big":1e+21,"normal":100,"small":1e-7}';

    sevenConditionsPass = tablesCount >= 30 && jcsPass;
    assertCheck('Cumplimiento de Condiciones de Aceptación (3 Fases, DDL, Propiedad, RLS y JCS)', sevenConditionsPass);
  } catch (err) {
    assertCheck('Cumplimiento de Condiciones de Aceptación (3 Fases, DDL, Propiedad, RLS y JCS)', false, err.message);
  } finally {
    await db.close();
  }

  // 6. Prueba del Migrador con SET ROLE app_owner, Advisory Lock y Fail-Closed
  let migrateCodePass = false;
  try {
    const code = fs.readFileSync('scripts/migrate-production.mjs', 'utf8');
    const hasSetRole = code.includes('SET ROLE app_owner;');
    const hasAdvisoryLock = code.includes('pg_advisory_lock');
    const hasFailClosed = code.includes('CHECKSUM_MISMATCH');
    migrateCodePass = hasSetRole && hasAdvisoryLock && hasFailClosed;
  } catch (err) {
    console.log('  -> Error comprobando migrador:', err.message);
  }
  assertCheck('Migrador con SET ROLE app_owner, Advisory Lock e Inspección Fail-Closed (C-02)', migrateCodePass);

  // 7. Pruebas HTTP Reales Fastify & Probes
  process.env.DATABASE_URL = 'postgresql://politica_canon_app:dummy@127.0.0.1:5432/politica_canon';
  process.env.REDIS_URL = 'redis://127.0.0.1:6379/0';
  process.env.SESSION_SECRET = '0123456789abcdef0123456789abcdef';
  process.env.APP_BASE_URL = 'https://peaceful-johnson.194-164-175-146.plesk.page';

  let probeTestPass = false;
  try {
    const { buildServer } = await import('./dist/server.js');
    const fastifyApp = buildServer();
    ...
    const homeRes = await fastifyApp.inject({ method: 'GET', url: '/' });

    const healthOk = healthRes.statusCode === 200 && healthRes.json().status === 'ok';
    const readyHandled = readyRes.statusCode === 503 || readyRes.statusCode === 200;
    const homeOk = homeRes.statusCode === 200 && homeRes.payload.includes('Intranet Privada');

    probeTestPass = healthOk && readyHandled && homeOk;
    await fastifyApp.close();
  } catch (err) {
    console.log('  -> Error probando servidor Fastify:', err.message);
  }
  assertCheck('Pruebas HTTP Reales (/healthz, /readyz, Portada Intranet Autenticada Nginx)', probeTestPass);

  console.log('\n---------------------------------------------------');
  console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
  if (failCount === 0) {
    console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.3: PASS');
  } else {
    console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.3: FAIL');
    process.exit(1);
  }
}
