 ...
 * Release Candidate: Política Canon v0.3.30 (Inmutable - Candidato de Gobernanza)
 */
...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.30 ===\n');
...
async function runValidation() {
  ...
  try {
    let zipPath = path.resolve(process.cwd(), 'politica-canon-v0.3.30.zip');
    let manifestPath = path.resolve(process.cwd(), 'MANIFEST_v0.3.30.json');

    if (!fs.existsSync(zipPath) || !fs.existsSync(manifestPath)) {
      zipPath = path.resolve(process.cwd(), '../politica-canon-v0.3.30.zip');
      manifestPath = path.resolve(process.cwd(), '../MANIFEST_v0.3.30.json');
    }
   ...
   └─ SHA-256: ${hashObserved.slice(0, 16)}..., Tamaño: ${zipBytes.length} bytes, Entradas: ${manifest.fileCount}, Raíz: politica-canon-v0.3.30/`);
    } else {
   ...
   └─ Re-calculados y validados ${verifiedCount} checksums SHA-256 individuales en RELEASE_FILES.json. 0 alteraciones.`);
    }
    passCount++;
  } catch (err) {
  ...
  try {
   ...
   └─ El script 'test' apunta a 'validate_v0.3.30.cjs' y todos los artefactos de scripts npm existen físicamente.`);
    passCount++;
  } catch (err) {
  ...
  // CHECK 4: Pruebas Ejecutables de Descifrado de Payloads Legacy v0.3.25 enc: (H-01, M-04 Autónomo)
  try {
  ...
  // CHECK 6: Coherencia Total de Metadatos de Release v0.3.30 (M-01)
  try {
    const pkg = JSON.parse(fs.readFileSync(path.resolve(process.cwd(), 'package.json'), 'utf8'));
    if (pkg.version !== '0.3.30') throw new Error(`package.json version es ${pkg.version}, se esperaba 0.3.30`);

    const filesToCheck = [
      { file: 'deploy/scripts/provision.sh', key: 'v0.3.30' },
      { file: 'src/server.ts', key: 'v0.3.30' },
      { file: 'public/index.html', key: 'v0.3.30' },
      { file: 'deploy/systemd/politica-canon.service', key: 'v0.3.30' },
      { file: 'deploy/systemd/politica-canon-outbox-worker.service', key: 'v0.3.30' },
      { file: 'deploy/plesk/vhost_nginx.conf', key: 'v0.3.30' },
      { file: 'scripts/migrate-production.mjs', key: 'v0.3.30' },
      { file: 'scripts/email-worker.mjs', key: 'v0.3.30' },
      { file: 'scripts/test-integration-pg16.mjs', key: 'v0.3.30' },
      { file: 'db/0000_bootstrap_roles.sql', key: 'v0.3.30' },
      { file: 'db/0002_bootstrap_permissions.sql', key: 'v0.3.30' },
    ];
    ...
    console.log(`✅ CHECK 6: Coherencia Total de Metadatos de Release v0.3.30 (M-01)
   └─ Versión 0.3.30 100% consistente en package.json, provision.sh, server.ts, index.html, systemd, nginx, DDLs y scripts.`);
    passCount++;
  } catch (err) {
  ...
  if (passCount === TOTAL_CHECKS) {
    console.log('DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.30: PASS (7/7 CONTROLES SUPERADOS)\n');
    process.exit(0);
  } else {
    console.error(`DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.30: FAIL (${passCount}/${TOTAL_CHECKS} CONTROLES SUPERADOS)\n`);
    process.exit(1);
  }
}
