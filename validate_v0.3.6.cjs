...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.6 (FASE 1 MVP) ===\n');
...
async function runValidation() {
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (47 Informes y Matrices Completo H-03/H-05)
  ...
  // 4. Coherencia de Metadatos de Release v0.3.6 (H-04)
  ...
  const metaVersion = packageJson.version === '0.3.6' &&
    nginxConf.includes('v0.3.6') &&
    systemdService.includes('v0.3.6') &&
    provisionSh.includes('v0.3.6') &&
    indexHtml.includes('v0.3.6') &&
    deploymentReport.includes('v0.3.6');

  assertCheck('Coherencia de Metadatos de Release v0.3.6 (H-04)', metaVersion, `package.json=${packageJson.version}`);

  // 5. Integración PostgreSQL PGlite: 3 Fases, Standalone ALTER DATABASE, Permisos DML, Audit Worker y Default Privileges (C-01, H-01, H-02)
  try {
    const db = new PGlite();
  ...
  // 6. Verificación del Migrador DDL Fail-Closed (C-01) y Bootstrap Post Non-Transactional con Verificación de Catálogo (C-01 v0.3.6)
  ...
  // 7. Servidor Fastify & Probes HTTP Reales /readyz (H-05)
  try {
  ...
  console.log(`DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.6: ${failCount === 0 ? 'PASS' : 'FAIL'}\n`);
