...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.7 (FASE 1 MVP) ===\n');
...
async function runValidation() {
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (49 Informes y Matrices Completo)
  ...
  // 4. Coherencia de Metadatos de Release v0.3.7 (H-04)
  ...
  const metaVersion = packageJson.version === '0.3.7' &&
    nginxConf.includes('v0.3.7') &&
    systemdService.includes('v0.3.7') &&
    provisionSh.includes('v0.3.7') &&
    indexHtml.includes('v0.3.7') &&
    deploymentReport.includes('v0.3.7');

  assertCheck('Coherencia de Metadatos de Release v0.3.7 (H-04)', metaVersion, `package.json=${packageJson.version}`);
  ...
  try {
    ...
    assertCheck('Cumplimiento de Condiciones de Aceptación (3 Fases, SET ROLE, Audit Dispatcher BYPASSRLS, DML Restringido)', dmlDenied && funcDenied);
  } catch (err) {
    assertCheck('Cumplimiento de Condiciones de Aceptación (3 Fases, SET ROLE, Audit Dispatcher BYPASSRLS, DML Restringido)', false, err.message);
  }
  ...
  // 7. Existencia y Registro de Runner de Integración test-integration-pg16.mjs
  const hasIntegrationScript = fs.existsSync('scripts/test-integration-pg16.mjs') &&
    packageJson.scripts['test:integration'] === 'node scripts/test-integration-pg16.mjs';

  assertCheck('Existencia y Registro de Runner de Integración Real (GATE-01)', hasIntegrationScript);

  console.log(`\n---------------------------------------------------`);
  console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
  console.log(`DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.7: ${failCount === 0 ? 'PASS' : 'FAIL'}\n`);
