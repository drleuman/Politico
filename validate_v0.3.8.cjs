...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.8 (FASE 1 MVP) ===\n');
...
async function runValidation() {
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (51 Informes y Matrices Completo)
  ...
  // 4. Coherencia de Metadatos de Release v0.3.8 (H-04)
  ...
  const metaVersion = packageJson.version === '0.3.8' &&
    nginxConf.includes('v0.3.8') &&
    systemdService.includes('v0.3.8') &&
    provisionSh.includes('v0.3.8') &&
    indexHtml.includes('v0.3.8') &&
    deploymentReport.includes('v0.3.8');

  assertCheck('Coherencia de Metadatos de Release v0.3.8 (H-04)', metaVersion, `package.json=${packageJson.version}`);
  ...
  // 7. Verificación de Invariante de Fail-Closed Estricto en test-integration-pg16.mjs (C-01, C-02, H-01)
  const integrationRunnerCode = fs.readFileSync('scripts/test-integration-pg16.mjs', 'utf8');
  const strictFailClosed = integrationRunnerCode.includes('REAL_PG16_AND_REDIS_REQUIRED') &&
    integrationRunnerCode.includes('ALTER ROLE politica_canon_app WITH PASSWORD') &&
    integrationRunnerCode.includes('finally') &&
    integrationRunnerCode.includes('docker compose -f docker-compose.audit.yml down -v') &&
    !integrationRunnerCode.includes('PGlite');

  assertCheck('Gate de Integración Estricta PG16/Redis Fail-Closed sin Fallback ni Monkey-Patching (C-01, C-02, H-01)', strictFailClosed);

  console.log(`\n---------------------------------------------------`);
  console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
  console.log(`DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.8: ${failCount === 0 ? 'PASS' : 'FAIL'}\n`);
