...
console.log('=== AUDITORÍA E INSPECCIÓN MECÁNICA Y SEMÁNTICA RIGUROSA — RELEASE v0.3.9 (FASE 1 MVP) ===\n');
...
async function runValidation() {
  ...
  // 2. Preservación Criptográfica Inmutable de Informes Históricos (53 Informes y Matrices Completo)
  ...
  // 4. Coherencia de Metadatos de Release v0.3.9 (H-04)
  ...
  const metaVersion = packageJson.version === '0.3.9' &&
    nginxConf.includes('v0.3.9') &&
    systemdService.includes('v0.3.9') &&
    provisionSh.includes('v0.3.9') &&
    indexHtml.includes('v0.3.9') &&
    deploymentReport.includes('v0.3.9');

  assertCheck('Coherencia de Metadatos de Release v0.3.9 (H-04)', metaVersion, `package.json=${packageJson.version}`);

  // 5. Integración PostgreSQL PGlite: 3 Fases, Standalone ALTER DATABASE, Permisos DML, Audit Worker y Default Privileges
  try {
    const db = new PGlite();
    ...
    // Verificación de Atributos de audit_dispatcher
    ...
    if (!dispatcherBypass) {
      throw new Error('audit_dispatcher no tiene BYPASSRLS activado.');
    }

    // Verificación de Propiedad de get_pending_outbox_tenants
    ...
    if (funcOwner !== 'audit_dispatcher') {
      throw new Error(`get_pending_outbox_tenants es propiedad de '${funcOwner}', se requiere 'audit_dispatcher'.`);
    }

    // Verificación de Denegación DML Directa a app_user sobre Tablas de Gobernanza
    ...
    // Verificación de Denegación de EXECUTE en get_pending_outbox_tenants para app_user
    ...
    assertCheck('Inicialización de 3 Fases y Permisos DML (PGlite Basal)', dmlDenied && funcDenied);
  } catch (err) {
    assertCheck('Inicialización de 3 Fases y Permisos DML (PGlite Basal)', false, err.message);
  }

  // 6. Verificación del Migrador DDL Fail-Closed y Bootstrap Post Non-Transactional con Verificación de Catálogo
  ...
  const standaloneDbOwnerFailClosed = postBootstrapCode.includes("ALTER DATABASE politica_canon OWNER TO app_owner;") &&
    postBootstrapCode.includes("process.exit(1)") &&
    postBootstrapCode.includes("pg_catalog.pg_get_userbyid(datdba)") &&
    !postPermissionsSql.includes("ALTER DATABASE politica_canon OWNER");

  assertCheck('Migrador DDL y Post-Bootstrap Fail-Closed con Aserción de Catálogo (C-01)', hasSetRole && standaloneDbOwnerFailClosed);

  // 7. Verificación de Inclusión Directa de Integración Real en npm test (C-01, C-02, C-03)
  const testScript = packageJson.scripts['test'] || '';
  const testScriptIncludesIntegration = testScript.includes('node scripts/test-integration-pg16.mjs');

  const integrationRunnerCode = fs.readFileSync('scripts/test-integration-pg16.mjs', 'utf8');
  const cleanFailClosedFinally = integrationRunnerCode.includes('REAL_PG16_AND_REDIS_REQUIRED') &&
    integrationRunnerCode.includes('throw new Error') &&
    !integrationRunnerCode.includes('process.exit(1);') && // no internal process.exit(1) inside try block
    integrationRunnerCode.includes('finally');

  assertCheck('Inclusión Directa de Integración Real en npm test y Garantía de Finally (C-01, C-02, C-03)', testScriptIncludesIntegration && cleanFailClosedFinally);

  console.log(`\n---------------------------------------------------`);
  console.log(`TOTAL CHECKS: ${passCount + failCount} | PASS: ${passCount} | FAIL: ${failCount}`);
  console.log(`DICTAMEN DE REMEDIACIÓN TÉCNICA RELEASE v0.3.9: ${failCount === 0 ? 'PASS' : 'FAIL'}\n`);
